...
/**
 * Plays one move for the human seat if possible and returns true; otherwise returns false.
 * We wait for the played card to leave the hand (or, for a pass, a brief moment) so as
 * not to re-click a stale card before the server has registered the move.
 */
...
/**
 * Plays the human seat through to the final standings screen. With bots at delay 0, a
 * full game runs in ~2 min via the UI. We vote "continue" at dealer boundaries
 * (a stop vote opens there automatically) to run the game through to its end.
 * We do not rely on any card value.
 */
